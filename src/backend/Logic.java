    ...
    public String runOperation(String userInput) {
        ...
        switch(command) {
            case ADD:
                ...
                taskAdd.setName(textToAdd);
                taskAdd.setDeadline(new Date("Wednesday", "300915"));
                taskAdd.setIsFloating(false);
                returnMessages = storage.addTask(taskAdd.getName(), taskAdd.getDeadline());
                break;
            case SEARCH:
                // Add a task and an event to return list
                Task taskSearch = new Task();
                taskSearch.setName("Some task name");
                taskSearch.setDeadline(new Date("Wednesday", "300915"));
                taskSearch.setIsFloating(false);
                returnMessages = "Task: " + taskSearch.getName() + CHAR_NEWLINE +
                        "Due on " + taskSearch.getDeadline().getDayString() + ", " +
                        taskSearch.getDeadline().getFormatDate() + ".";
                ...
                returnMessages = "Event: " + event.getName() + CHAR_NEWLINE +
